...
or classifications based on pre-defined instructions and scales 
...
# llm_relevance_name and llm_relevance_score is created as a new docvar in the corpus
# the checkpoint_file allow resuming the process in case of interruptions 
# without re-generating all responses
```
...
# llm_scale_score and llm_scale_evidence is created as a new docvar in the corpus
# the checkpoint_file allow resuming the process in case of interruptions 
# without re-generating all responses
```
...
corpus <- corpus %>%
  quanteda.tidy::mutate(validated = ai_validate(text, llm_scale_score))
